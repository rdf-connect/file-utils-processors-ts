...
export type GetMyClassT<C extends Processor<unknown>> =
    C extends Processor<infer T> ? T & C : unknown;
export class TestClient extends reexports.RunnerClient {
    next: (
        stream: reexports.ClientReadableStream<reexports.DataChunk>,
    ) => unknown;
    ...
    receiveStreamMessage(): reexports.ClientReadableStream<reexports.DataChunk> {
        const stream =
            new reexports.ClientReadableStreamImpl<reexports.DataChunk>(
                (data: Buffer) => {
